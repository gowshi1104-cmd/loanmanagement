...
const PaymentForm = ({
  ...
  const [cashfreePaymentSessionId, setCashfreePaymentSessionId] = useState("");
  ...
  useEffect(() => {
    window.history.pushState({ paymentForm: true }, "", window.location.href);
  ...
  const loadMemberDetails = async (loanId) => {
    try {
      ...
      const fetchedDueAmount = Number(data?.dueAmount ?? data?.due ?? 0);
      ...
      const fetchedOverdueAmount = Number(
        data?.overdueAmount ?? data?.overDueAmount ?? data?.overdue ?? 0,
      );
      ...
      const fetchedEmiAmount = Number(
        data?.emiAmount ?? data?.emi ?? data?.installmentAmount ?? 0,
      );
  ...
  const handlePaymentModeChange = (e) => {
    ...
    setPayment((prev) => ({
      ...
      accountNumber: value === "BANK_TRANSFER" ? prev.accountNumber : "",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        receivedBy:
          payment.paymentMode === "CASH" ? payment.receivedBy.trim() : null,
  ...
  const checkPaymentStatus = async (paymentId, showToast = true) => {
    try {
      if (showToast) {
  ...
  const handleTriggerUpiPayment = async () => {
    ...
    if (!window.Cashfree) {
      toast.error("Cashfree Checkout is still loading. Please try again.");
    ...
    try {
      ...
      console.log("CREATING CASHFREE LOCAL PAYMENT:", payload);

      const createResponse = await createPayment(payload);

      const createdPayment = createResponse?.data || createResponse;

      console.log("LOCAL PAYMENT RESPONSE:", createdPayment);

      const paymentId = createdPayment?.paymentId || createdPayment?.id;

      if (!paymentId) {
        throw new Error("Payment ID was not returned by backend");
      }
      ...
      console.log("CREATING CASHFREE ORDER FOR PAYMENT:", paymentId);
      ...
      if (!order?.paymentSessionId) {
        throw new Error("Cashfree payment session ID was not returned");
      }
      ...
      if (checkoutResult?.error) {
        ...
        throw new Error(
          checkoutResult.error?.message || "Cashfree checkout failed",
        );
      }
  ...
  const fieldsDisabled = paymentSuccess || paymentProcessing;
    ...
    <>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow p-6">
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-green-700 mt-1">
                  Payment has been successfully verified by Cashfree.
                </p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mt-4">
                  <div className="bg-white rounded-xl border border-green-200 p-3">
                    ...
                    <p className="font-semibold text-slate-900 mt-1">
                      ₹ {Number(emiAmount).toLocaleString("en-IN")}
                    </p>
                  </div>
                  ...
                  <div className="bg-white rounded-xl border border-green-200 p-3">
                    ...
                    <p className="font-semibold text-slate-900 mt-1 break-all">
                      {cashfreePaymentId || createdPaymentId || "-"}
                    </p>
                  </div>
                ...
                <div className="flex flex-wrap gap-3 mt-4">
                  <button
                    ...
                    className="px-4 py-2.5 rounded-xl bg-green-600 text-white font-medium hover:bg-green-700 flex items-center gap-2"
                  >
                    <Receipt size={18} />
                    View Receipt
                  </button>
                  ...
                  <button
                    ...
                    className="px-4 py-2.5 rounded-xl border border-green-300 text-green-700 font-medium hover:bg-green-100 flex items-center gap-2"
                  >
                    <Printer size={18} />
                    Print Receipt
                  </button>
          ...
          <div className="mb-6 rounded-2xl border border-yellow-200 bg-yellow-50 p-5">
            <div className="flex items-start gap-4">
              <div className="w-12 h-12 rounded-full bg-yellow-100 flex items-center justify-center shrink-0">
                <Loader2 size={26} className="text-yellow-600 animate-spin" />
              </div>
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-yellow-700 mt-1">
                  Payment was initiated, but Cashfree has not confirmed the
                  final status yet.
                </p>

                <p className="text-sm text-yellow-700 mt-2">
                  Payment ID: <strong>{createdPaymentId || "-"}</strong>
                </p>

                {cashfreeOrderId && (
                  <p className="text-sm text-yellow-700 mt-1">
                    Order ID: <strong>{cashfreeOrderId}</strong>
                  </p>
                )}
                ...
                <button
                  ...
                  className="mt-4 px-4 py-2.5 rounded-xl bg-yellow-600 text-white font-medium hover:bg-yellow-700 disabled:opacity-50 flex items-center gap-2"
                >
                  <RefreshCw size={18} />
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-red-700 mt-1">
                  {paymentFailureMessage || "Payment could not be completed."}
                </p>
                ...
                <button
                  ...
                  disabled={paymentProcessing}
                  className="mt-4 px-4 py-2.5 rounded-xl bg-red-600 text-white font-medium hover:bg-red-700 disabled:opacity-50 flex items-center gap-2"
                >
                  <RefreshCw size={18} />
                  Retry Payment
                </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <input
              ...
              className={`w-full border rounded-xl px-4 py-3 outline-none ${
                errors.loanId ? "border-red-500" : "border-slate-300"
              } ${fieldsDisabled ? "bg-slate-100 cursor-not-allowed" : ""}`}
            />
            ...
            {errors.loanId && (
              <p className="text-xs text-red-500 mt-1">{errors.loanId}</p>
            )}
          </div>
          ...
          <div>
            ...
            <input
              ...
              className="w-full border border-slate-300 bg-slate-100 rounded-xl px-4 py-3 cursor-not-allowed"
            />
          ...
          <div>
            ...
            <input
              type="text"
              value={`₹ ${Number(dueAmount || 0).toLocaleString("en-IN")}`}
              disabled
              className="w-full border border-slate-300 bg-slate-100 rounded-xl px-4 py-3 cursor-not-allowed"
            />
          </div>
          ...
          <div>
            ...
            <input
              type="text"
              value={`₹ ${Number(overdueAmount || 0).toLocaleString("en-IN")}`}
              disabled
              className="w-full border border-red-200 bg-red-50 text-red-700 rounded-xl px-4 py-3 cursor-not-allowed"
            />
          </div>
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-2">
              EMI Amount <span className="text-blue-500 text-xs">(Auto)</span>
            </label>

            <input
              type="text"
              value={`₹ ${Number(emiAmount || 0).toLocaleString("en-IN")}`}
              disabled
              className="w-full border border-blue-200 bg-blue-50 text-blue-700 rounded-xl px-4 py-3 cursor-not-allowed"
            />
        ...
        {payment.paymentMode === "UPI" && (
          <div className="mt-6 border border-blue-200 bg-slate-50 rounded-2xl p-6">
            <div className="flex items-center gap-3 mb-6">
              <div className="w-11 h-11 rounded-xl bg-blue-100 flex items-center justify-center">
                <QrCode size={23} className="text-blue-600" />
              </div>
            ...
            <div className="rounded-2xl border border-blue-200 bg-white p-5">
              <div className="flex items-start gap-4">
                <div className="w-12 h-12 rounded-xl bg-blue-50 flex items-center justify-center shrink-0">
                  <Smartphone size={23} className="text-blue-600" />
                </div>
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-600 mt-1 leading-6">
                    Click the button below to open Cashfree Checkout.
                  </p>

                  <p className="text-sm text-slate-600 leading-6">
                    Cashfree will provide the available UPI payment options for
                    the customer.
                  </p>
                </div>
              </div>
              ...
              <div className="mt-5 grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="rounded-xl bg-blue-50 border border-blue-200 p-4">
                  ...
                  <p className="font-semibold text-blue-700 mt-1">
                    ₹ {Number(emiAmount || 0).toLocaleString("en-IN")}
                  </p>
                </div>
              </div>
              ...
              <div className="flex justify-end mt-5">
                <button
                  ...
                  className="px-6 py-3 rounded-xl bg-blue-600 text-white font-medium hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                >
                  {paymentProcessing ? (
                    <>
            ...
            {paymentCreated && createdPaymentId && (
              <div className="mt-5 rounded-xl border border-yellow-200 bg-yellow-50 p-4">
                <div className="flex items-start gap-3">
                  <Loader2
                    size={20}
                    className="text-yellow-600 animate-spin mt-0.5"
                  />
        ...
        {payment.paymentMode === "CASH" && (
          <div className="mt-6 border border-slate-200 rounded-2xl p-6 bg-slate-50">
            <h3 className="font-semibold text-slate-900 mb-4">Cash Payment</h3>
            ...
            {errors.receivedBy && (
              <p className="text-xs text-red-500 mt-1">{errors.receivedBy}</p>
            )}
          </div>
        )}
        ...
        {paymentSuccess && (
          <div className="mt-6 rounded-xl border border-green-200 bg-green-50 px-4 py-3">
            <p className="text-sm text-green-700 font-medium">
              🔒 Payment completed successfully. Payment details are locked and
              cannot be edited.
            </p>
          </div>
        )}
        ...
        <div className="border-t mt-8 pt-6 flex justify-end gap-3">
          ...
          <button
            ...
            className="px-5 py-3 border border-slate-300 rounded-xl hover:bg-slate-50 flex items-center gap-2"
          >
            <X size={18} />
            Cancel
          </button>
          ...
          >
            <div className="bg-blue-600 text-white px-6 py-5">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-blue-100 mt-1">Payment Receipt</p>
                </div>
            ...
            <div className="p-6">
              ...
              <div className="border rounded-xl overflow-hidden">
                ...
                <div className="flex justify-between px-4 py-3 border-b">
                  <span className="text-sm text-slate-500">Due Amount</span>
                ...
                <div className="flex justify-between px-4 py-3 border-b">
                  ...
                  <span className="font-medium text-red-600">
                    ₹ {Number(overdueAmount).toLocaleString("en-IN")}
                  </span>
                </div>
                ...
                <div className="flex justify-between px-4 py-3 border-b">
                  ...
                  <span className="font-medium text-slate-900 text-right break-all ml-4">
                    {cashfreePaymentId || createdPaymentId || "-"}
                  </span>
                </div>
        ...
        <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-black/50 px-4">
          <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl p-6">
            ...
            <p className="text-sm text-slate-500 mt-2 leading-6">
              You have unsaved changes in this payment form. If you leave this
              page, all the changes you made will be lost.
            </p>
